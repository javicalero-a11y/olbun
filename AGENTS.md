...
|                           |                                                                                                                                                                                      |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Milestones complete**   | M0 Foundation · M1 Identity & tenancy · M2 Audit & shell (bar i18n) · M3 Deadline engine · M4 Contracts · M5 Expedientes · M6 Communications · M7 Detection · M8 Connected mailboxes |
| **Milestone in progress** | M9 — Documents. M10 risk/incidents already has a working vertical slice, but is not complete.                                                                                        |
...
| Coverage is ratcheted at the measured M8 baseline (60.48% lines, 61.26% functions, 50.31% branches, 60.91% statements), below SPEC's 80% target. New work must not lower it; M23 closes the remaining legacy service/action gap (ADR 0008)                                                                           | M23        |
...
| Documents are stored unscanned: `estadoAnalisis` is always PENDIENTE and the screen says "sin analizar" rather than implying clean. Antivirus is the next M9 slice                                                                                                                                                   | M9         |
| Text extraction covers text files, PDFs and `.docx`. Scanned PDFs and the old binary `.doc` are recorded as unindexed and say so on screen; scans need OCR                                                                                                                                                           | M9         |
| No PDF index in the zip export — the index and chronology are Markdown                                                                                                                                                                                                                                               | M9         |
| No antivirus: uploads stay PENDIENTE and are labelled unscanned                                                                                                                                                                                                                                                      | M9         |
| Deletion is exposed only as the retention purge, which requires an expired period. There is still no way to delete a document on request, and no way to _set_ `bloqueadoPorLitigio` from the interface — the purge honours it, but only a seed or a migration can turn it on                                         | M9         |
