...
|                           |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Milestones complete**   | M0 Foundation · M1 Identity & tenancy · M2 Audit & shell (bar i18n) · M3 Deadline engine · M4 Contracts · M5 Expedientes · M6 Communications · M7 Detection · M8 Connected mailboxes · M9 Documents · M10 Risks and incidents · M11 Personal, convenio y adscripción                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **Milestone in progress** | M13 — Jornada. Registro diario append-only con encadenamiento de hash, cálculo de nocturnas y extra, bolsa del límite anual y pantalla con comprobación de la cadena. Faltan la importación desde terminales y apps, y el informe ITSS. M12 — Primera tanda entregada 2026-08-15: ausencias con sus 17 tipos, días naturales y laborables, índice de absentismo y cobertura real por categoría. Entregado 2026-08-16. Quedan fuera, con dueño: reasignar arrastrando en el planificador (necesita equivalente por teclado, M23) y las señales de §4.4 que dependen de hitos posteriores — nómina, jornada y subrogación. M11 closed 2026-08-14: 1334 unit and real-Postgres integration tests, plus 160 E2E against a production build (chromium and mobile-chrome), all green. Migration replay from an empty database is still unverified — it needs a destructive reset nobody has authorised. |
...
- **Mutations use `tenantTransaction()`, never `tenantClient().$transaction()`.**
  Prisma query extensions that call client-level methods leave an interactive
  transaction. The explicit helper keeps `set_config`, scoped queries and audit
  events on one connection (ADR 0005).
...
| The integration suite uses a real isolated Postgres, but it is not provisioned automatically through Testcontainers yet                                                                                                                                                                                              | M23        |
| Redis/BullMQ is not yet in Docker Compose; MinIO/S3 is present and used for immutable evidence objects                                                                                                                                                                                                               | M12        |
| Coverage is ratcheted at the measured M10 baseline (63.89% lines, 66.28% functions, 53.57% branches, 63.61% statements), below SPEC's 80% target. New work must not lower it; M23 closes the remaining legacy service/action gap (ADR 0008)                                                                          | M23        |
| El registro de jornada es _tamper-evident_, no _tamper-proof_: detecta la manipulación, no la impide frente a quien tenga acceso de propietario a la base de datos. Anclar la cadena fuera del sistema es lo que daría lo segundo                                                                                    | M19        |
| Migration replay against an empty database has never been run: the chain is only known to work incrementally on a database that has followed it all along. Verifying it costs a `prisma migrate reset`, which destroys the dev data and needs an explicit go-ahead                                                   | M12        |
| Health endpoint checks Postgres only; Redis and S3 readiness checks still need adding                                                                                                                                                                                                                                | M12 / M23  |
| The magic-link round trip is not covered end to end: Auth.js stores only a hash of the token, so a test cannot rebuild the emailed link. Needs a mail-capture seam                                                                                                                                                   | M23        |
...
| SMTP transport exists, but a deployment still needs verified sender/domain credentials and `SMTP_URL`                                                                                                                                                                                                                | Deployment |
| HSTS and `upgrade-insecure-requests` are enabled only when `APP_URL`/`AUTH_URL` uses HTTPS. Plain-HTTP LAN previews must never emit them. A browser pinned by an older build may still require clearing its HSTS state.                                                                                              | —          |
| Forwarding aliases are implemented and protected by a provider-neutral inbound webhook, but public MX/domain routing still requires choosing and configuring an email provider                                                                                                                                       | Deployment |
| Manual uploads still require pressing «Analizar»; connected mailboxes do run detection automatically. This keeps an upload failure separate from a paid model call and is now an explicit product choice                                                                                                             | —          |
| Real Google Workspace and Microsoft 365 consent is not exercised in CI: it requires provider tenants, verified credentials and an HTTPS deployment. Provider contracts are covered with mocked HTTP plus real database integration tests                                                                             | Deployment |
| Connected mailbox synchronisation and Claude batch result collection are manual. Incremental cursors are complete; polling jobs and Gmail/Graph push notifications arrive with BullMQ                                                                                                                                | M12        |
| Claude Message Batches is optional and is not a Zero Data Retention service. A customer's approved DPIA/DPA must permit that transfer; without `ANTHROPIC_API_KEY`, Olbun keeps correspondence local                                                                                                                 | Deployment |
| OCR is explicit and capped per request; automatic scheduling, retries and continuation beyond `OCR_MAX_PAGES` arrive with BullMQ. The stored `OCR_PARCIAL` state never claims the remaining pages were read                                                                                                          | M12        |
| The old binary `.doc` format is retained and labelled unsupported. It needs a reliable sandboxed converter before its contents can enter the evidence index                                                                                                                                                          | M23        |

### Local environment

Three checkouts of this repository share one Postgres and one MinIO on ports
5433 and 9000, and a dev server started from any of them answers on 3100. A
dev server can therefore serve a **different codebase** than the one being
edited, which is not visible from the browser. Check with
`lsof -a -p $(lsof -ti:3100) -d cwd -Fn` before concluding that a change did
not take effect.

The E2E suite locally attaches to whatever already listens on 3100
(`reuseExistingServer`), so `playwright.config.ts`'s `env` block does **not**
apply. Two consequences: `.env` must carry the same `INBOUND_EMAIL_SECRET` the
suite has written into it, and the provider test in `incorporacion.spec.ts`
only passes against a production build, because a dev server offers the Google
and Microsoft buttons whether or not credentials exist. Stop the dev server to
have Playwright build and start its own.

`APP_URL` and `AUTH_URL` point at the LAN address in the local env files so the
app can be browsed from another device. That makes the server issue absolute
redirects to `192.168.1.199`, and any E2E navigation that starts at `localhost`
and gets redirected then crosses origins and aborts — it looks like an auth
regression and is not one. Run the suite with both pinned:

```bash
APP_URL=http://localhost:3100 AUTH_URL=http://localhost:3100 pnpm test:e2e
```
